function CoordinatorNotes({ value, onChange, error }) {
  return (
    <div>
      <label htmlFor="coordinator-notes" className="text-[11px] font-semibold tracking-wide text-slate-500">
        COORDINATOR NOTES
      </label>
      <textarea
        ...
        aria-describedby={error ? "coordinator-notes-error" : "coordinator-notes-help"}
        rows="5"
      ...
      <div className="mt-1 flex justify-between gap-4 text-xs text-slate-400">
        <span id="coordinator-notes-help">Add relevant details from the outreach call.</span>
        <span>{value.length} characters</span>
      </div>
      {error && <p id="coordinator-notes-error" role="alert" className="mt-2 text-xs text-red-600">{error}</p>}
    </div>
  );
}
